		...
		<div class="row">
			<div class="col-xs-14 col-sm-12 col-md-11">
				<div class="panel panel-warning">
					...
					<form action="${pageContext.request.contextPath}/admin/addContract"
						method="POST">
						<div class="col-md-6 mb-6" style="margin-top: 15px;">
							<label>Nhân viên</label> <select class="ui dropdown form-control"
								id="SelectEmployee" name="employee">
								<c:forEach items="${employee}" var="employee">
									<option value="${employee.id}">${employee.id}-
										${employee.name}</option>
								</c:forEach>
							</select> <label>${errorEmployee}</label>
						</div>
						<div class="col-md-6 mb-6" style="margin-top: 15px;">
							<label>Lương</label> <input type="text" class="form-control"
								name="salary" id="salary" value="${salary}" placeholder="Lương">
						...
						<div class="col-md-6 mb-3" style="margin-top: 15px;">
							...
							<div class="ui calendar" id="rangeend">
								<input type="text" class="form-control datepicker" id="endDate"
									value="${expirationday}" name="expirationday"
									placeholder="Ngày kết thúc"> <label
									style="color: red; margin-top: 5px;">${errorexpirationday}</label>
							</div>
						</div>
						<br />
						<div style="width: 100%; text-align: center; margin-top: 16%;">
							...
							<button type="submit" class="btn btn-success"
								style="float: left; margin-left: 41%;">Lưu</button>
						</div>
					</form>
					<button class="btn btn-warning" id="close"
						style="background: #aaa; border: #aaa; margin-left: 20px;">Hủy
						bỏ</button>
				</div>
			</div>
		</div>
