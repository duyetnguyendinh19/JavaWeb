									...
									<div class="col-12 col-sm-12 col-xs-12 col-md-12 col-lg-8">
										...
										<select
											...
											<option value="0" ${month == 0 ? 'selected' : ''}
												${empty month ? 'selected' : '' }>Tất cả tháng</option>
											<option value="1" ${month == 1 ? 'selected' : ''}
												${empty month ? 'selected' : '' }>Tháng 1</option>
										...
										<div class="col-12 col-sm-12 col-xs-12 col-md-12 col-lg-4">
											...
											<input type="text" name="year" id="year"
												...
												placeholder="Năm..." style="border-radius: 5px !important;"
												value="${year}" />
										</div>
										...
										<c:forEach items="${listSalary}" var="listSalary"
											...
											<tr style="text-align: center;">
												...
												<td><fmt:formatNumber type="number"
														maxIntegerDigits="10" value="${listSalary.total}" /></td>
...
<script>
	...
	if (isFormatNumber !== false) {
		$('#year').on(
				'keyup',
				function() {
					var formattedVal = formatNumberString($('#year').val()
							.replace(/,/g, ''));
			...
			.keypress(
					function(event) {
						...
						if (event.keyCode === 13) {
							if (number < 1) {
								window.location
										.assign("http://localhost:9999/Manager/admin/listSalary/" + 1);
							} else if (number > numbertotal) {
								window.location
										.assign("http://localhost:9999/Manager/admin/listSalary/"
												+ numbertotal);
							} else {
								window.location
										.assign("http://localhost:9999/Manager/admin/listSalary/"
